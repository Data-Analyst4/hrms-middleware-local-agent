...
uses Unit_Enroll, Unit_SysInfo, Unit_Log, Unit_BellInfo, Unit_LockCtrl, Unit_Prtcode, Utils,
  Unit_TrMode, Unit_AccessTz, Unit_Department, Unit_Proxy,
...
begin
    ...
    begin
        ...
        if StrLen(PChar(string(lpszDeviceUUID))) = 16 then
        begin
          gMachineNumber := SBXPC1.ConnectP2p(lpszDeviceUUID, lpszIPAddr, StrToInt(txtPortNo.Text), StrToInt(txtPassword.Text), nError);
        ...
        else if SBXPC1.ConnectTcpip(gMachineNumber, lpszIPAddr, StrToInt(txtPortNo.Text), StrToInt(txtPassword.Text)) then
        begin
            mOpenFlag := true;
            cmdOpen.Enabled := false;
            cmdClose.Enabled := true;
