...
begin
    ...
    vEnrollNumber := StrToInt(txtEnrollNumber.Text);
    vFingerNumber := StrToInt(cmbBackupNumber.Text);
    if not chkDisable.Checked then vFlag := 1
    else vFlag := 0;
    vRet := bpc.EnableDevice(gMachineNumber, 0);
    if Not vRet then
    begin
...
begin
  ...
  strXML := MakeXMLCommandHeader(bpc, 'GetUserInfo');
  bpc.XML_AddInt(strXML, 'UserID', vEnrollNumber);
  ...
  begin
    ...
    txtUserTz1.Text := IntToStr(bpc.XML_ParseInt(strXML, 'Timezone1'));
    txtUserTz2.Text := IntToStr(bpc.XML_ParseInt(strXML, 'Timezone2'));
  end
  else
  begin
    bpc.GetLastError(vErrorCode);
...
begin
  ...
  strXML := MakeXMLCommandHeader(bpc, 'SetUserInfo');
  bpc.XML_AddInt(strXML, 'UserID', vEnrollNumber);
  bpc.XML_AddInt(strXML, 'Timezone1', vTimezone1);
  bpc.XML_AddInt(strXML, 'Timezone2', vTimezone2);
  bpc.XML_AddInt(strXML, 'GroupNo', vGroupNo);
...
begin
  ...
  strXML := MakeXMLCommandHeader(bpc, 'GetUserPeriod');
  bpc.XML_AddInt(strXML, 'UserID', lUserID);

  bRet := bpc.GeneralOperationXML(strXML);
  if bRet then
  begin
    chkUsePeriod.Checked := (bpc.XML_ParseInt(strXML, 'Used') = 1);
    lStartDate := bpc.XML_ParseInt(strXML, 'Start');
    lEndDate := bpc.XML_ParseInt(strXML, 'End');
...
begin
  ...
  lUserID := StrToInt(txtEnrollNumber.Text);
  strXML := MakeXMLCommandHeader(bpc, 'SetUserPeriod');
  bpc.XML_AddInt(strXML, 'UserID', lUserID);
  if (chkUsePeriod.Checked) then
  begin
    bpc.XML_AddInt(strXML, 'Used', 1);
  end
  else
  begin
    bpc.XML_AddInt(strXML, 'Used', 0);
  end;
  ...
  bpc.XML_AddInt(strXML, 'Start', lStartPeriod);
  bpc.XML_AddInt(strXML, 'End', lEndPeriod);
...
begin
    ...
    vRet := bpc.ModifyDuressFP(gMachineNumber, vEnrollNumber, vFingerNumber, vDuressSetting);
    if vRet then
    begin
      lblMessage.Caption := 'Success!';
    end
