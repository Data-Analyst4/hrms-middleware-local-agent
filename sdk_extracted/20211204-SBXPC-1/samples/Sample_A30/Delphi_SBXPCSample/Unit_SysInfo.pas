...
begin
    ...
    vRet := bpc.GetDeviceTime(gMachineNumber, vYear, vMonth, vDay, vHour, vMinute, vSecond, vDayOfWeek);
    if vRet then
    begin
...
begin
    ...
    vRet := bpc.SetDeviceTime(gMachineNumber);
...
begin
    ...
    lblMessage.Caption := 'Waiting...';
    vRet := bpc.PowerOffDevice(gMachineNumber);
    if vRet then lblMessage.Caption := 'Success!'
    else
    begin
        bpc.GetLastError(vErrorCode);
...
begin
    ...
    lblMessage.Caption := 'Waiting...';
    if chkEnableDevice.Checked then vFlag := 1
    else vFlag := 0;
    vRet := bpc.EnableDevice(gMachineNumber, vFlag);
    if vRet then
    begin
        if vFlag = 1 then
        begin
...
begin
    ...
    vRet := bpc.GetDeviceInfo(gMachineNumber, vInfo, vValue);
    if vRet then
    begin
        case vInfo of
...
begin
    ...
    vRet := bpc.SetDeviceInfo(gMachineNumber, vInfo, vValue);
    if vRet then
    begin
...
begin
    ...
    vRet := bpc.GetDeviceStatus(gMachineNumber, vStatus, vValue);
    if vRet then
    begin
        case vStatus of
