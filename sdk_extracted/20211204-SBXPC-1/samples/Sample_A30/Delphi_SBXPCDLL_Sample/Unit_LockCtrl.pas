...
begin
    ...
    strXML := MakeXMLCommandHeader('GetDoorStatusMulti');
    _XML_AddInt(strXML, 'DoorNo', lDoorNumber);

    vRet := _GeneralOperationXML(gMachineNumber, strXML);
    if vRet then
    begin
        vStatus := _XML_ParseLong(strXML, 'DoorStatus');
        case vStatus of
...
begin
    ...
    strXML := MakeXMLCommandHeader('SetDoorStatusMulti');
    _XML_AddInt(strXML, 'DoorNo', lDoorNumber);
    _XML_AddInt(strXML, 'DoorStatus', 3);
...
begin
    ...
    strXML := MakeXMLCommandHeader('SetDoorStatusMulti');
    _XML_AddInt(strXML, 'DoorNo', lDoorNumber);
    _XML_AddInt(strXML, 'DoorStatus', 4);
...
begin
    ...
    strXML := MakeXMLCommandHeader('SetDoorStatusMulti');
    _XML_AddInt(strXML, 'DoorNo', lDoorNumber);
    _XML_AddInt(strXML, 'DoorStatus', 5);
    vRet := _GeneralOperationXML(gMachineNumber, strXML);
...
begin
    ...
    strXML := MakeXMLCommandHeader('SetDoorStatusMulti');
    _XML_AddInt(strXML, 'DoorNo', lDoorNumber);
    _XML_AddInt(strXML, 'DoorStatus', 6);
...
begin
    ...
    strXML := MakeXMLCommandHeader('GetDoorParam');
    _XML_AddInt(strXML, 'DoorNo', lDoorNumber);

    bRet := _GeneralOperationXML(gMachineNumber, strXML);
    if bRet then
    begin
      cmbDoorSensorType.ItemIndex := _XML_ParseInt(strXML, 'DoorSensorType');
      txtLockReleaseTime.Text := IntToStr(_XML_ParseInt(strXML, 'LockReleaseTime'));
      txtDoorOpenTimeout.Text := IntToStr(_XML_ParseInt(strXML, 'DoorOpenTimeout'));
      cmbAntipassUsage.ItemIndex := 1 - _XML_ParseInt(strXML, 'DoorOpenTimeout');
      cmbAntipassNo.ItemIndex := _XML_ParseInt(strXML, 'AntipassNo');
      cmbAntipassLocation.ItemIndex := _XML_ParseInt(strXML, 'Location');
...
begin
  ...
  lDoorNumber := StrToInt(txtDoorNumber.Text);
  strXML := MakeXMLCommandHeader('SetDoorParam');
  _XML_AddInt(strXML, 'DoorNo', lDoorNumber);

  _XML_AddInt(strXML, 'DoorSensorType', cmbDoorSensorType.ItemIndex);
  _XML_AddInt(strXML, 'LockReleaseTime', StrToInt(txtLockReleaseTime.Text));
  _XML_AddInt(strXML, 'DoorOpenTimeout', StrToInt(txtDoorOpenTimeout.Text));
  _XML_AddInt(strXML, 'UseAntipass', 1 - cmbAntipassUsage.ItemIndex);
  _XML_AddInt(strXML, 'AntipassNo', cmbAntipassNo.ItemIndex);
  _XML_AddInt(strXML, 'Location', cmbAntipassLocation.ItemIndex);
...
begin
    ...
    strXML := MakeXMLCommandHeader('GetUnlockgroup');

    bRet := _GeneralOperationXML(gMachineNumber, strXML);
    if bRet then
    begin
      lInfo := Integer(addr(unlockgroups));
      _XML_ParseBinaryLong(strXML, 'UnlockGroupBinary', lInfo, 2 * 10);
      for i:= 0 to 19 do
      begin
...
begin
  ...
  lInfo := Integer(addr(unlockgroups));
  strXML := MakeXMLCommandHeader('SetUnlockgroup');
  _XML_AddBinaryLong(strXML, 'UnlockGroupBinary', lInfo, 2 * 10);
